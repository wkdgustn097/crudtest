...
<!-- addStudentForm,Action,deleteStudentAction,
	updateStudentForm, updateStudentAction-->
...
<%
   ...
   WHERE s.student_no = ?
   */
   int studentNo = Integer.parseInt(request.getParameter("studentNo"));// request.getParameter("studnetNo");
...
<body>
   ...
   <div>
   	<button><a href="<%=request.getContextPath()%>/updateStudentForm.jsp?studentNo=<%=(Integer)(map.get("studentNo"))%>">수정</a></button>
	<button><a href="<%=request.getContextPath()%>/deleteStudentAction.jsp?studentNo=<%=(Integer)(map.get("studentNo"))%>">삭제</a></button>
	<button><a href="<%=request.getContextPath()%>/studentList.jsp">돌아가기</a></button>
   </div>
</body>
</html>